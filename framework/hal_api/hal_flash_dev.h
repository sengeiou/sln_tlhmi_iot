...
/**
 * @brief declare the flash dev ##name
 */
#define HAL_FLASH_DEV_DECLARE(name) int HAL_FlashDev_##name##_Register();

/**
 * @brief register the flash dev ##name
 */
#define HAL_FLASH_DEV_REGISTER(name, ret)                             \
    {                                                                 \
        ret = HAL_FlashDev_##name##_Register();                       \
        if (ret != 0)                                                 \
        {                                                             \
            LOGE("HAL_FlashDev_%s_Register error %d", "##name", ret); \
            return ret;                                               \
        }                                                             \
    }
