...
static sln_flash_status_t _lfs_cleanupHandler(const flash_dev_t *dev, unsigned int timeout_ms)
{
    ...
    lfs_fs_traverse(&s_LittlefsHandler.lfs, _lfs_traverse_create_used_blocks, &usedBlocks);

    startTime = FWK_CurrentTimeUs();

    /* find next block starting from free.i */
    for (int i = 0; i < LFS_SECTORS; i++)
    {
        currentTime = FWK_CurrentTimeUs();
        /* Check timeout */
    ...
    LOGI("%i empty_blocks starting from %i available in %ims", emptyBlocks, s_LittlefsHandler.lfs.free.i,
         (FWK_CurrentTimeUs() - startTime) / 1000);
...
static sln_flash_status_t _lfs_writeHandler(const flash_dev_t *dev, const char *path, void *buf, unsigned int size)
{
    ...
    res = lfs_file_opencfg(&s_LittlefsHandler.lfs, &file, path, LFS_O_CREAT|LFS_O_RDWR, &s_FileDefault);
    if (res)
    {
