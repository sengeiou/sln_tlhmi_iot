 ...
 * @brief Version
 */
#define FWK_MAJOR_VERSION 2
#define FWK_MINOR_VERSION 0

/**
 * @brief init the framework manager ##name
 */
#define FWK_MANAGER_INIT(name, ret)                      \
    {                                                    \
        ret = FWK_##name##_Init();                       \
        if (ret != 0)                                    \
        {                                                \
            LOGE("FWK_%s_Init error %d", "##name", ret); \
            return ret;                                  \
        }                                                \
    }

/**
 * @brief start the framework manager ##name
 */
#define FWK_MANAGER_START(name, priority, ret)            \
    {                                                     \
        ret = FWK_##name##_Start(priority);               \
        if (ret != 0)                                     \
        {                                                 \
            LOGE("FWK_%s_Start error %d", "##name", ret); \
            return ret;                                   \
        }                                                 \
    }
...
typedef enum _status_framework_groups
{
    ...
    kStatusFrameworkGroups_Multicore       = 9,
...
#define MULTICORE_MANAGER_TASK_NAME  "multicore_manager"
#define MULTICORE_MANAGER_TASK_STACK 1024

/*
 * Recomend frame task priority
 *
...
/* Defines max static messages to be allocated */

#define MULTICORE_MAX_MESSAGES 5
#ifndef FWK_SUPPORT_MULTICORE
#define FWK_SUPPORT_MULTICORE 1
